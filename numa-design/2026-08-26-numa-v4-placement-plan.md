...
- **G2-locality:** unpinned local-refill share (`/numa/span-refills/*`) ≥90% at every GOMAXPROCS in {2, 8, 32, 128, 256} (E1's numbers were 75/73/53/62/57%). *Protocol clarification (recorded 2026-08-26, before the gate battery, after stage-4 Task P4 diagnostics):* the share is the **steady-state counter delta** — workload runs a ≥5s warmup, counters are snapshotted, the workload continues for the measured window, and the delta is the gate reading. Rationale: the counters are process-lifetime and the startup ramp (window arming, first stream growth, cache priming) dilutes short windows — 75%→86%→89.9% at 2s/5s/15s ramp-inclusive at 256P — while the gate's real subjects (minutes-long pathology workloads, the IMC physical-traffic gate) are steady-state by construction. The ramp-inclusive share is reported alongside as exploratory, never gated. The bar itself is unchanged.
...
**Stage 4 (node-aware page allocation):** *(amended 2026-08-26 after the
stage-4 design review's rev-1 REJECTED verdict — findings C1/C2/M1–M5; the
amendment lands with the rev-2 design, before any implementation, per this
section's own pre-registration rule. Original text superseded where it
conflicts.)*

- **P8 — Shape:** `pageAlloc` stays ONE structure (no per-node pageAlloc instances). Node awareness enters as a **windowed search**: per-node address windows computed in mallocinit from the ACTUAL hint addresses (longest contiguous run at the layout's real spacing — `randomizeHeapBase` is baseline-ON: 256 GiB spacing, mod-256 prefix wrap can trim or invalidate one stream's window per launch), a per-window searchAddr obeying the same inUse invariant as `p.searchAddr` (maxSearchAddr sentinel when unarmed/exhausted), and `allocNode(npages, node)` doing at most one windowed find. **Fallback order for a routed direct allocation (amended per review M1, supersedes "falling back to today's unrestricted search on failure"):** windowed alloc → homed grow ONCE (latching the node's window off permanently if growth lands outside it) → windowed retry ONCE → unconditional unrestricted fallback. Free/scavenge paths untouched beyond per-window searchAddr lowering (gated on `numaHeapHomingActive`).
- **P9 — Callers routed:** allocSpan with `typ == spanAllocHeap` ONLY (review M4b: allocManual/stack/workbuf untouched), and only under a **syscall-free node key** (review M3): an explicit node argument (mcentral refill/grow path) or the placement P-home; Auto-keyed callers without placement stay as lazy as today — no getcpu at allocSpan frequency. Node-aware page-cache fill (`allocToCacheNode`) with fallback to plain `allocToCache`.
- **P10 — Cost guard:** ≤1 windowed find + ≤1 homed grow + ≤1 retry per allocSpan call; sentinel makes steady-state misses O(1); `find`/`alloc`/`allocToCache` stay byte-identical (new code in a separate file, referenced only from goexperiment-gated call sites, DCE'd from the off binary — review M5); the 1P alloc micro ≤ +2% hard gate is measured **against stock** on the combined stage-2+4 tree (the standing +3.73% must clear or the stage fails).
- **G4:** G2 battery re-run on the combined tree (G2-primary bar unchanged) AND large-object locality (arena-tag vs allocating P's home via test hook) ≥90% AND **G4-RSS (added per review M2): peak-RSS-bytes on the G2-primary garbage arms, experiment vs stock, ≤ +10% (benchstat, same session)** — the homed-grow-first ordering trades reuse for locality and this gate bounds the footprint cost — plus the standing hard gates.
