...
func TestNUMAFillOneSocketConfined(t *testing.T) {
	...
	if mode == 0 {
		// MPOL_DEFAULT means numaSetProcessBindAll never ran at all in
		// the child, which only happens when the testprog binary itself
		// was built without GOEXPERIMENT=numa (e.g. the experiment set
		// for the test binary but not exported to the environment) -- a
		// build/invocation issue, not a confinement regression, so Skip
		// rather than Fail.
...
func TestNUMAConfineSkipsNarrowedAffinity(t *testing.T) {
	...
	// and the narrowed 2-CPU mask is left untouched. The BIND-all task
	// policy is affinity-independent and still applies, so the task
	// policy is MPOL_BIND (mode=2).
	...
	if mode != 2 {
		t.Fatalf("mode=%d want MPOL_BIND(2) (BIND-all task policy)", mode)
	}
}
...
func TestNUMAStandDownOnGOMAXPROCSGrowth(t *testing.T) {
	...
	// Same rationale as TestNUMAFillOneSocketConfined: an
