...
// numaMaxHeapNodes is 1 with the experiment off: every
// numaMaxHeapNodes-sized array (mheap.arenaHints, mheap.curArena, and
// the per-node mcentral spanSets) collapses to a single
...
// heapArena.node: 0 with the experiment off. A [0]uint8
...
// sizeof(heapArena) stays at its pre-NUMA value off-build. Access
