...
// ParseCPUListIntoNodeMap exposes parseCPUListIntoNodeMap's truncation
// result without exposing Topology's representation details.
func ParseCPUListIntoNodeMap(data []byte) (n int, truncated bool, err error) {
	var dst [maxCPUs]int8
	return parseCPUListIntoNodeMap(&dst, 0, data)
}
