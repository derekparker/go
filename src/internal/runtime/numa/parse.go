...
//
// CPU ids >= maxCPUs are skipped and reported through truncated.
func parseCPUListIntoNodeMap(dst *[maxCPUs]int8, nodeID int8, data []byte) (n int, truncated bool, err error) {
	i := bytealg.IndexByte(data, '\n')
	if i < 0 {
		return 0, false, errMalformedFile
	}
